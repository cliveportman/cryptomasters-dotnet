...
{
    /* Challenge 7: Provided with a Base64-encoded text file which has been encrypted using
     * AES-128 in ECB mode under the key "YELLOW SUBMARINE", decrypt the text.
     */
    // I've made the class abstract as there's no need to instantiate it.
    // I don't see this as necessary but Rider seems to like it.
    public abstract class Challenge7
    {
        ...
        public static string Run()
        {
            try
            {
                var lines = File.ReadAllLines(Path.Combine(Utility.Assets, "challenge-7.txt"));
